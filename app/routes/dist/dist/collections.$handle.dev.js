...
exports.meta = function (_a) {
  ...
  var data = _a.data;
  return [{
    title: "Hydrogen | " + ((_b = data === null || data === void 0 ? void 0 : data.collection.title) !== null && _b !== void 0 ? _b : '') + " Collection"
  }];
};
    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        ...
        case 1:
          ...
          return [4
          /*yield*/
          , axios_1["default"].get('https://services.mybcapps.com/bc-sf-filter/search/collections?shop=avida-healthwear-inc.myshopify.com&q=' + handle).then(function (collection_response) {
              ...
              return __generator(this, function (_a) {
                switch (_a.label) {
                  case 0:
                    ...
                    return [4
                    /*yield*/
                    , axios_1["default"].get('https://services.mybcapps.com/bc-sf-filter/filter?shop=avida-healthwear-inc.myshopify.com&build_filter_tree=true&collection_scope=' + collectionId, {
                      timeout: 10000
                    }).then(function (r) {
        ...
        case 3:
          ...
          return [2
          /*return*/
          , remix_runtime_1.defer(__assign(__assign(__assign({}, deferredData), criticalData), {
            products: products,
            filters: filters,
            handle: handle
          }))];
    ...
    return __generator(this, function (_b) {
      switch (_b.label) {
        case 0:
          ...
          return [4
          /*yield*/
          , Promise.all([storefront.query(COLLECTION_QUERY, {
            variables: __assign({
              handle: handle
            }, paginationVariables)
          })])];
...
function FilterList(el, onchange, filters, handle, query) {
  function dd(document, tag) {// document.querySelector('#filters-form').submit();
  }

  return React.createElement("div", null, React.createElement("div", {
    className: 'form-group'
  }, React.createElement("p", null, React.createElement("label", {
    className: "cb-container link"
  }, React.createElement("span", null, el.el), React.createElement("input", {
    type: "checkbox",
    name: "tags",
    value: el.el,
    onChange: el.onchange,
    defaultChecked: el.query.includes(el.el)
...
function Collection() {
  ...
  var _a = react_1.useLoaderData(),
      collection = _a.collection,
      menu = _a.menu,
      allItems = _a.allItems,
      products = _a.products,
      filters = _a.filters,
      handle = _a.handle; // console.log("products:");
  // console.log(products);
  // const checkedStates = new Array(filters.length).fill({tag: "TheTest", checked: false});


  var checkedStates = [];
  var minPrice = Math.min.apply(Math, products.map(function (prod) {
    return prod.price_min;
  }));
  var maxPrice = Math.max.apply(Math, products.map(function (prod) {
    return prod.price_min;
  }));
  ;
  ...
  var priceInputChange = function priceInputChange(e) {
    var priceInput = document.querySelectorAll(".price-input input");
    var rangeInput = document.querySelectorAll(".range-input input");
    var range = document.querySelector(".slider .progress");
    var priceGap = 1;
    var minPrice = parseInt(priceInput[0].value); //let maxPrice = parseInt(priceInput[1].value);
    // if (maxPrice - minPrice >= priceGap && maxPrice <= rangeInput[1].max) {
  ...
  var rangeChange = function rangeChange(e) {
    var priceInput = document.querySelectorAll(".price-input input");
    var rangeInput = document.querySelectorAll(".range-input input");
    var range = document.querySelector(".slider .progress");
    var priceGap = 1; // let minPrice = parseInt(priceInput[0].value);

    var maxPrice = parseInt(priceInput[0].value); // let minVal = parseInt(rangeInput[0].value);
    ...
    if (maxVal - 0 < 0) {
      ...
      priceInput[0].value = maxVal; // range.style.left = (minVal / rangeInput[0].max) * 100 + "%";
  ...
  var ShowElement = function ShowElement(el, enabledFilters) {
    var included = true;

    if (enabledFilters.length) {
      included = el.tags.some(function (val) {
        return enabledFilters.includes(val) && el.collections.some(function (c) {
      ...
      included = el.collections.some(function (c) {
        return c.handle == collection.handle;
      });
    }
    ...
    if (included) {
      if (priceFilter) {
        included = el.variants.some(function (val) {
          return parseFloat(val.price) <= parseFloat(priceFilter);
        });
      }
    }

    return included; // return enabledFilters.length ?
  ...
  return React.createElement("div", {
    ...
    className: 'left-side collection-filters ' + (openFilters ? 'active' : '')
  }, React.createElement("div", {
    className: 'filters-close',
    onClick: closeFilters
  }, "X"), React.createElement("form", {
    method: 'get',
    action: "/collections/" + handle + "?search",
    id: "filters-form",
    onChange: function onChange(e) {
      return submit(e.currentTarget);
    }
  }, filters.map(function (el) {
    return React.createElement(React.Fragment, null, " ", React.createElement("h2", null, el.label), el.values.map(function (item) {
      return React.createElement(FilterList, {
        el: item,
        onchange: submit,
        filters: checkedStates,
        handle: handle,
        query: enabledFilters
      });
    }));
  }), React.createElement("h2", null, "Price"), React.createElement("section", null, React.createElement("div", {
    className: "price-input"
  }, React.createElement("div", {
    className: "field"
  }, React.createElement("span", null, "Max"), React.createElement("input", {
    type: "number",
    className: "input-max",
    ...
    id: "filterIconTitle"
  }, "Filter"), " ", React.createElement("path", {
    d: "M10 12.261L4.028 3.972h16L14 12.329V17l-4 3z"
  }), " ")), " Filters"), React.createElement("section", {
    className: "collection-intro"
  }, React.createElement("div", {
    className: 'right-side'
  }, collection.image && React.createElement("div", {
  ...
  }, "Shop ", collection.title.toLowerCase()), React.createElement("p", {
    className: "collection-description"
  }, collection.description)))), React.createElement("div", {
    className: "collection-products"
  }, products.map(function (el) {
    return ShowElement(el, enabledFilters) && React.createElement(SingleItem, {
      item: el,
...
function SingleItem(_a) {
  var item = _a.item,
      collections = _a.collections;
  return React.createElement("div", {
    className: "product-link"
  }, React.createElement("div", null, React.createElement("div", {
    className: 'slider-item single-item',
    key: item.id
  }, React.createElement(react_1.Link, {
    className: "product-link",
    key: item.id,
    prefetch: "intent",
    to: "/products/" + item.handle
  }, React.createElement("div", {
    className: 'slider-upper-block',
    style: {
      backgroundImage: 'url(' + (item.images && Object.keys(item.images).length > 0 ? item.images[Object.keys(item.images)[0]] : "") + ')',
      backgroundSize: 'contain',
      backgroundRepeat: 'no-repeat',
      backgroundPosition: 'center'
    }
  }, React.createElement(hydrogen_1.Image, {
    ...
    className: 'slider-lower-block'
  }, React.createElement("h4", null, item.title), React.createElement("span", {
    className: 'price'
...
function ProductItem(_a) {
  ...
  return React.createElement(react_1.Link, {
    className: "product-item",
    key: product.id,
    prefetch: "intent",
    to: variantUrl
  }, product.featuredImage && React.createElement(hydrogen_1.Image, {
    alt: product.featuredImage.altText || product.title,
    aspectRatio: "1/1",
    data: product.featuredImage,
    loading: loading,
    sizes: "(min-width: 45em) 400px, 100vw"
  }), React.createElement("h4", null, product.title), React.createElement("small", null, React.createElement(hydrogen_1.Money, {
